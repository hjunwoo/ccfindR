...
read_10x <- function(dir, count='matrix.mtx', genes='genes.tsv', 
  ...
  if(remove.zeros){
    null_row <- rowSums(Mat)==0; nr <- sum(null_row)
    null_col <- colSums(Mat)==0; nc <- sum(null_col)
    if(nr > 0){
      glist <- glist[!null_row,]
      Mat <- Mat[!null_row,]
      cat(nr,'empty rows removed\n')
    }
    if(nc > 0){ 
      clist <- clist[!null_col,]
      Mat <- Mat[,!null_col]
      cat(nc,'empty columns removed\n')
    }
  }
  scNMFSet(count=Mat, genes=glist, cells=clist, 
...
filter_cells <- function(object, umi.min=0, umi.max=Inf, plot=TRUE){
  ...
  umi.count <- colSums(object@count)
...
filter_genes <- function(object, markers= NULL, vmr.min=0, 
  ...
  selected_genes <- variable_genes <- rep(FALSE, nrow(object@genes))
  if(!is.null(markers)){
    for(k in 1:ncol(object@genes))
      selected_genes <- selected_genes | (mk <- object@genes[,k] %in% markers)
    marker_genes <- selected_genes
  } 
    
  ncexpr <- rowSums(object@count>0) # no. of cells expressed in each gene
  object <- object[ncexpr>0,]
  ncexpr <- ncexpr[ncexpr>0]
  gmean <- rowMeans(object@count)
  ngenes <- dim(object)[1]
  if(!save.memory) var <- rowVars(object@count,means=gmean) # memory intensive
  else{
    if(progress.bar){
      cat('Calculating vmr...\n')
    ...
    var <- c()
    denom <- ncol(object@count)
    for(i in 1:nrow(object@count)){
      g <- object@count[i,]-gmean[i]
  ...
  variable_genes <- vmr.min <= vmr & 
            min.cells.expressed <= ncexpr & ncexpr <= max.cells.expressed
  if(rescue.genes & sum(variable_genes)<ngenes){
    mode_genes <- c()
    cat('Scanning genes for those with modes...\n')
    if(progress.bar) pb <- utils::txtProgressBar(style=3)
    for(i in 1:ngenes){
      if(variable_genes[i]) mode <- FALSE
      else mode <- has_mode(object@count[i,])
  ...
  object@count <- object@count[selected_genes,]
  tmp <- as.data.frame(object@genes[selected_genes,])
  names(tmp) <- names(object@genes)
  object@genes <- tmp
  object
}
...
rowVars <- function(x, means){
  if(missing(means)) means <- rowMeans(x)
  rowSums((x-means)^2)/(dim(x)[2]-1)
...
gene_map <- function(object, rank, markers=NULL, subtract.mean=TRUE,
  ...
  if(missing(rank)) rank <- object@ranks[1] # by default the first rank
  rid <- which(object@ranks == rank)
  w <- object@basis[[rid]]
  if(subtract.mean){
    if(log) w <- log10(w)
    w <- w - rowMeans(w)
...
cell_map <- function(object, rank, main = 'Cells', ...){
  ...
  if(missing(rank)) rank <- object@ranks[1]
  rid <- which(object@ranks == rank)
  h <- object@coeff[[rid]]
  nrow <- dim(h)[1]
...
visualize_clusters <- function(object, rank, verbose=TRUE, cex=1, 
                               cex.names = 0.7, ...){
  ...
  else rid <- which(object@ranks == rank)
