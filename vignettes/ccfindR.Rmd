...
of count data and factorizes the data matrix $X$ into two matrices $W$ and 
$H$ [@lee_seung]:
...
```
$ R CMD INSTALL ccfindR_0.99-0.tar.gz
```
...
genes <- as(ABC, 'DataFrame')
rownames(genes) <- ABC
cells <- as(abc, 'DataFrame')
rownames(cells) <- abc
...
One may read a `MatrixMarket` format file directly:
```{r, eval=T}
# read sparse matrix
mat <- Matrix::readMM('pbmc/matrix.mtx')
sc <- scNMFSet(count = mat, rowData = as(1:nrow(mat), 'DataFrame'),
               colData = as(1:ncol(mat), 'DataFrame'))
sc
```
...
```{r, cache=T}
sc <- factorize(sc, ranks = 3:7, nrun = 5, verbose = 1, progress.bar = FALSE)
```
...
```{R, fig.width=6.5, fig.height=3, fig.cap='Factorization quality measures as functions of the rank. Dispersion measures the degree of bimodality in consistency matrix. Cophenetic correlation measures the degree of agreement between consistency matrix and hierarchical clustering.'}
plot(sc)
```
...
```{r, cache=T}
sb <- vb_factorize(sb, ranks = 2:7, nrun = 5, verbose = 1, Tol = 5e-4, progress.bar = FALSE)
```
...
gene_map(sb, markers = markers, rank = 5, max.per.cluster = 4, gene.name = rowData(sb)[,2],
         cexRow = 0.7)
```
...
Based on the marker-metagene map in Fig. 6, we rename the clusters 1-5 
as follows:
```{r}
cell_type <- c('NK','B_cell','Monocytes','CD4+_T','CD8+_T')
...
Fig. 9, the relative distance between clusters can be seen to be consistent 
